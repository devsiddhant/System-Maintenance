...
echo "Updating system packages with zypper..." | tee -a "$LOG_FILE"
sudo zypper refresh >> "$LOG_FILE" 2>&1
sudo zypper -n update >> "$LOG_FILE" 2>&1
...
echo "Cleaning zypper caches..." | tee -a "$LOG_FILE"
...
if [ -n "${TELEGRAM_BOT_TOKEN:-}" ] && [ -n "${TELEGRAM_CHAT_ID:-}" ]; then
  curl -s -X POST "https://api.telegram.org/bot${TELEGRAM_BOT_TOKEN}/sendDocument" \
    ...
    -F document=@"${LOG_FILE}" \
    -F caption="OpenSUSE update log $(basename "$LOG_FILE")" >> "$LOG_FILE" 2>&1
fi
