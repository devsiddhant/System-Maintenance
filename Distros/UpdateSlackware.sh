...
echo "Updating Slackware packages with slackpkg..." | tee -a "$LOG_FILE"
sudo slackpkg update >> "$LOG_FILE" 2>&1
sudo slackpkg upgrade-all >> "$LOG_FILE" 2>&1
...
# Slackware doesn't have a dedicated cache clean command like others.
...
if [ -n "${TELEGRAM_BOT_TOKEN:-}" ] && [ -n "${TELEGRAM_CHAT_ID:-}" ]; then
  curl -s -X POST "https://api.telegram.org/bot${TELEGRAM_BOT_TOKEN}/sendDocument" \
    ...
    -F document=@"${LOG_FILE}" \
    -F caption="Slackware update log $(basename "$LOG_FILE")" >> "$LOG_FILE" 2>&1
fi
