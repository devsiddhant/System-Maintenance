...
echo "Updating Void Linux packages with xbps..." | tee -a "$LOG_FILE"
sudo xbps-install -Suy >> "$LOG_FILE" 2>&1
...
echo "Cleaning xbps caches..." | tee -a "$LOG_FILE"
sudo xbps-remove -O >> "$LOG_FILE" 2>&1
...
if [ -n "${TELEGRAM_BOT_TOKEN:-}" ] && [ -n "${TELEGRAM_CHAT_ID:-}" ]; then
  curl -s -X POST "https://api.telegram.org/bot${TELEGRAM_BOT_TOKEN}/sendDocument" \
    ...
    -F document=@"${LOG_FILE}" \
    -F caption="Void Linux update log $(basename "$LOG_FILE")" >> "$LOG_FILE" 2>&1
fi
